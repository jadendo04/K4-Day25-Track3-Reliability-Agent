    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open":
                ...
                recovery_times_ms.append((entry["ts"] - open_ts) * 1000)  # type: ignore[operator]
                open_ts = None
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        # - primary_timeout_100: primary is fully down, so fallback must carry almost all traffic
        # - primary_flaky_50: circuit should still keep availability high via fallback
        # - all_healthy / default: no fallback needed, availability should be near-perfect
